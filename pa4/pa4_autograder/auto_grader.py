...
#                print "result file is " + resultfile
            ...
            if resultfile[len(resultfile)-5] == "1" :
                try:
                    run_command("./%s 128 assoc:4 lru 4 %s > %s"%(dirname, input_dir + "/" + testfile,resultfile), timeout=runtime[dirname], user_program=True, echo=True);
                except:
                ...
                try:
                    run_command("./%s 64 assoc:8 lru 2 %s > %s"%(dirname, input_dir + "/" + testfile,resultfile), timeout=runtime[dirname], user_program=True, echo=True);
                except:
                    print "An exception occurred while executing %s %s"%(dirname, testfile)
                    os.chdir(prevdir)
                    print "myScore is ", myscore
